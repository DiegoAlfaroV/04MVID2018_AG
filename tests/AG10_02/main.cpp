...
void render(const Shader& lightingShader, const Shader& stencilShader, const uint32_t cubeVAO,  const uint32_t quadVAO, const uint32_t tex1, const uint32_t tex2) {
	...
	model = glm::mat4(1.0f);
	model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.2f, 1.0f));
	model = glm::scale(model, glm::vec3(0.41f, 0.41f, 0.41f));
...
int main(int argc, char* argv[]) {
	...
	uint32_t text1 = createTexture("../tests/AG10_02/albedo.png");
	uint32_t text2 = createTexture("../tests/AG10_02/specular.png");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		render(lightingShader, stencilShader, cubeVAO, quadVAO, text1, text2);
